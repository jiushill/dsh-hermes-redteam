...
//#endregion
// ===== Wire-level prefill: only on the absolute first turn =====
function injectWirePrefill(messages) {
	var i, last;
	if (messages.length === 0) return;
	last = messages[messages.length - 1];
	if (last.role !== "user" && last.role !== "tool") return;
	// Only inject when NO assistant has ever spoken. This guards against
	// tool-call loop re-injection and multi-turn pollution.
